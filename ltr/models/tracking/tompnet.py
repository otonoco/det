...
class ToMPnet_DeT(nn.Module):
    ...
    def __init__(self, feature_extractor, feature_extractor_depth, head, head_layer, merge_type, W_rgb, W_depth):
        super().__init__()
        ...
        if self.merge_type == 'conv':
            ## need to recheck the size of conv network
...
def tompnet50(filter_size=4, head_layer='layer3', backbone_pretrained=True, head_feat_blocks=0, head_feat_norm=True,
    ...
    # ToMP network
    net = ToMPnet_DeT(feature_extractor=backbone_net, feature_extractor_depth=backbone_net_depth, head=head, head_layer=head_layer, merge_type=merge_type, W_rgb=W_rgb, W_depth=W_depth)
    return net
...
def tompnet101(filter_size=1, head_layer='layer3', backbone_pretrained=True, head_feat_blocks=0, head_feat_norm=True,
    ...
    # ToMP network
    net = ToMPnet_DeT(feature_extractor=backbone_net, head=head, head_layer=head_layer)
    return net